...
describe("Escrow", function () {
  ...
  describe("Buy", function () {
    ...
    it("should allow to buy ", async () => {
      ...
      const tokensPrice = rodoBalance.div(2).mul(price).div(1e2);
      const fee = tokensPrice.mul(feePercentage).div(1e5);
      const requiredEth = tokensPrice.add(fee);
    ...
    it("should give the required eth", async () => {
      ...
      const tokensPrice = rodoBalance.div(4).mul(price).div(1e2);
      const fee = tokensPrice.mul(feePercentage).div(1e5);
      const expectedRequiredEth = tokensPrice.add(fee);
